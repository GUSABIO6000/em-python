...
class RouterNATManager:
    ...
    def send_command(self, command, wait=1.5):
        if self.shell is None:
    ...
    def add_nat(self):
        ...
        for cmd in commands:
            ...
            output = self.send_command(cmd, wait=0.8)
    ...
    def show_all_nats(self):
        ...
        output = self.send_command("sh ip nat translations", wait=2.5)
    ...
    def close_active_sessions(self):
        ...
        output = self.send_command("show users", wait=1.5)
        ...
        if other_sessions:
            ...
            if messagebox.askyesno("Cerrar sesiones adicionales", message):
                ...
                final_output = self.send_command("show users", wait=1.5)
                ...
                if messagebox.askyesno("Cerrar sesión actual", message):
                    ...
                    try:
                        if 'vty' in current_session.lower():
                            ...
                            self.send_command(f"clear line {current_session}", wait=0.8)
    ...
    def send_command_from_cli(self, event):
        ...
        output = self.send_command(command, wait=1.5)
